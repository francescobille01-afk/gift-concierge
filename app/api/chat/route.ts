...
const tools: Anthropic.Messages.ToolUnion[] = [
  ...
  },
  {
    name: "propose_gifts",
    description:
      "Present a curated list of 4 to 6 gift suggestions. Always provide at least 4; use 5 or 6 only when every additional product is a strong match.",
    input_schema: {
      type: "object",
      properties: {
        suggestions: {
...
Using ALL available data — recipient profile, occasion, budget, interests, notes — decide between 4 and 6 specific, real, well-known products. Always return at least 4. Add a fifth or sixth only when each is genuinely strong; quality matters more than filling all 6 slots. Be fully committed: a real brand + model that genuinely exists and is commonly sold on ${loc.amazonDomain}. Draw on what you already know about real products — do NOT search the web. Each product must be the result of intersecting ALL signals together (e.g. "loves specialty coffee + minimalist aesthetic + €50 budget + birthday = Fellow Stagg EKG kettle"), not just one signal in isolation.

▸ DISTRIBUTE BY INTEREST: cover every meaningful interest when possible, but give more space to the strongest signals. Do not add filler just to make the distribution even.

▸ HOW TO READ THE PER-INTEREST DETAIL LINES (format "Interest: what | where | level | brand"). These four signals are your product-selection algorithm — apply them to every gift:
...
Call \`propose_gifts\` with 4 to 6 suggestions. Never return fewer than 4 or more than 6, and never end the conversation without calling it.
...
▸ BUDGET IS A HARD CONSTRAINT, NOT A SUGGESTION: every product's price must sit inside ${loc.currencySymbol}${recipient.budgetMin}–${loc.currencySymbol}${recipient.budgetMax} (this window is already derived from the buyer's stated budget: at most 30% below it, at most 15% above it).
  ...
  ✗ DO NOT PAD or DO NOT INFLATE. Two failure modes to avoid: (a) filling the list with cheap little gadgets just because the interest has cheap products — if the budget is ${loc.currencySymbol}${recipient.budgetMin}–${loc.currencySymbol}${recipient.budgetMax} and the interest's typical items are cheap, choose FEWER categories of NICER, genuinely more expensive single items (e.g. cooking on a ${loc.currencySymbol}80 budget → a real stand mixer / premium chef's knife / cast-iron pot, NOT a list of ${loc.currencySymbol}15 utensils); (b) writing a fake inflated price on a product that really costs far less just to fit the window (e.g. calling a ${loc.currencySymbol}35 water bottle "${loc.currencySymbol}95"). If a product's real price is below the window, pick a genuinely pricier product — never lie about the price.
...
▸ 4 TO 6 ONLY: the final list must contain at least 4 and at most 6 products. Return 4 when the evidence is narrow, 5 for a balanced search, and 6 only when all six choices are excellent. Never use filler to reach 6.

▸ PERFECT FIT ONLY: every product must be tightly, obviously matched to THIS specific person's interests, sub-interest detail, level, and occasion. No filler and no "safe generic" picks.
...
• Give 4–6 NEW suggestions — do not just repeat the same items, and do not repeat anything rated 1–4.
...
export async function POST(req: NextRequest) {
  try {
      ...
      if (response.stop_reason === "tool_use") {
        ...
        for (const block of assistantContent) {
          ...
          if (block.name === "propose_gifts") {
            ...
            const proposedSuggestions = (input.suggestions ?? []).slice(0, 6);
            const hasMinimumSuggestions = proposedSuggestions.length >= 4;
            if (hasMinimumSuggestions) finalSuggestions = proposedSuggestions;
            toolResults.push({
              type: "tool_result",
              tool_use_id: block.id,
              content: JSON.stringify(hasMinimumSuggestions
                ? { ok: true, count: proposedSuggestions.length }
                : { ok: false, error: "You must call propose_gifts again with at least 4 and at most 6 strong suggestions." }),
              ...(hasMinimumSuggestions ? {} : { is_error: true }),
            });
          }
        }
    ...
    if (finalSuggestions.length < 4) {
      throw new Error("The model returned fewer than 4 gift suggestions");
    }
